...
package cachingsecretskeymanager
...
func TestNewErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mockClient := &mockSecretMgr{} // Use a mock client here too
			_, _, err := newWithClient(tt.cache, tt.reg, tt.cfg, mockClient)
			if err == nil {
...
func TestInsertKeyset(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			keyset: &model.Keyset{
				UniqueKeyID:    "unique2",
				SigningPrivate: "test-signing-private-2",
				EncrPrivate:    "test-encr-private-2",
			},
			mockSecret: &mockSecretMgr{
				createSecret: func(ctx context.Context, req *secretmanagerpb.CreateSecretRequest, opts ...gax.CallOption) (*secretmanagerpb.Secret, error) {
					return &secretmanagerpb.Secret{}, nil
...
func TestKeyset(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			mockCache: &mockCache{
				get: func(ctx context.Context, key string) (string, error) {
					...
					return string(expectedPayload), nil
				},
				set: func(ctx context.Context, key string, value string, ttl time.Duration) error { return nil },
			},
			mockSecret: &mockSecretMgr{
				accessSecretVersion: func(ctx context.Context, req *secretmanagerpb.AccessSecretVersionRequest, opts ...gax.CallOption) (*secretmanagerpb.AccessSecretVersionResponse, error) {
					t.Fatal("AccessSecretVersion should not be called on cache hit")
					return nil, nil
				},
			},
			expected: expectedKeyset,
		},
		{
			...
			mockSecret: &mockSecretMgr{
				...
				addSecretVersion: func(ctx context.Context, req *secretmanagerpb.AddSecretVersionRequest, opts ...gax.CallOption) (*secretmanagerpb.SecretVersion, error) {
					return &secretmanagerpb.SecretVersion{}, nil // Called for caching the SM response
				},
			},
			expected: expectedKeyset,
		},
		...
		},
		{
			...
			mockCache: &mockCache{
				...
				set: func(ctx context.Context, key string, value string, ttl time.Duration) error {
					return fmt.Errorf("cache set failure") // Simulate cache set failure
				},
			},
			mockSecret: &mockSecretMgr{
				accessSecretVersion: func(ctx context.Context, req *secretmanagerpb.AccessSecretVersionRequest, opts ...gax.CallOption) (*secretmanagerpb.AccessSecretVersionResponse, error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("Keyset() unexpected error = %v", err)
			}
			if retrievedKeyset == nil {
				t.Fatalf("Keyset() returned nil keyset but no error")
			}
			if retrievedKeyset.UniqueKeyID != tt.expected.UniqueKeyID ||
				retrievedKeyset.SigningPrivate != tt.expected.SigningPrivate ||
				retrievedKeyset.EncrPrivate != tt.expected.EncrPrivate ||
				retrievedKeyset.SigningPublic != tt.expected.SigningPublic ||
				retrievedKeyset.EncrPublic != tt.expected.EncrPublic {
				t.Errorf("Keyset() got = %v, want %v", retrievedKeyset, tt.expected)
			}
		})
	}
}
